...
function __construct($config ='rest'){
    ...
    function index_get(){
        ...
        if ($id_kalender == '') {
            # code...
            $kalender['result']=$this->db->get('tbl_kalender')->result();
        }else {
    ...
    function akademik_get(){
        ...
        $sql = "select id_kalender , nama_bulan , tanggal , nama_kalender , id_sekolah  from tbl_kalender inner join tbl_bulan on tbl_kalender.id_bulan = tbl_bulan.id_bulan where YEAR(tanggal) = year(now()) and id_sekolah = ? order by tanggal asc ";
        $query = $this->db->query($sql,array($id_sekolah))->result();
        $data = array();
